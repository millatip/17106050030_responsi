...
class Model_login extends CI_Model {
	...
	public function cekLogin()
	{
		...
		$queryAnggota = $this->db->get_where('anggota', array('username' => $username));
		$resultAnggota = $queryAnggota->row();
		...
		if ($queryAdmin->num_rows() > 0) {
			$password_decDb = $resultAdmin->password;
		...
		elseif ($queryPegawai->num_rows() > 0) {
			$password_decDb =  	($resultPegawai->password);
			if ($password_decDb == $password) {
				$array = array(
		...
		elseif ($queryAnggota->num_rows() > 0) {
			$password_decDb =  	($resultAnggota->password);
			if ($password_decDb == $password) {
				$array = array(
					'username' 	=>	$resultAnggota->nim,
					'level'		=>  'anggota',
					'nama' 		=>	$resultPegawai->nama,
					'foto'		=>  $resultPegawai->foto
					);				
